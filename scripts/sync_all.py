...
실행하면:
  1. 시장 지수 일별 데이터    (market_daily)
  2. 매크로 지표              (daily_macro, monthly_macro, quarterly_macro)
  3. 외국인/기관/개인 수급    (market_flow_daily)
  4. 종목 컨센서스            (stock_consensus)
  5. 미국 주식 재무제표       (financial_metric, US)
  6. 한국 주식 재무제표       (financial_metric, KR) — DART_API_KEY 있을 때만
  7. 대시보드 빌드            (docs/dashboard.html)

Usage
-----
# 전체 실행 (US 재무제표 + KR 컨센서스 포함)
    python scripts/sync_all.py

# KR 재무제표도 포함 (DART_API_KEY 필요, 느림 ~수 분)
    python scripts/sync_all.py --with-kr-financials --year 2024

# 대시보드 빌드 건너뜀
    python scripts/sync_all.py --skip-dashboard

# 빠른 모드 (컨센서스·재무제표 생략, 시장/매크로/수급만)
...
def _run(label: str, cmd: list[str], skip: bool = False) -> bool:
    ...
    print(f"[sync_all] {status}  {label}")
    return ok


def _has_dart_key() -> bool:
    return bool(os.getenv("DART_API_KEY", "").strip() or os.getenv("OPEN_DART_API_KEY", "").strip())
...
def main() -> None:
    ...
    for label, ok in results:
        mark = "✅" if ok else "❌"
        print(f"  {mark}  {label}")
    print(f"\n  총 {len(results)}단계 · 성공 {ok_count} · 실패/생략 {fail_count}")
    if fail_count > 0:
        print("  ❗ 실패 단계는 위 로그를 확인하세요.")
    else:
        print("  🎉 모든 단계 완료!")
    print("=" * 60)
