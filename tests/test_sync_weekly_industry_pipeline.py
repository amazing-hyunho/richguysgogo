...
class SyncWeeklyIndustryPipelineTests(unittest.TestCase):
    def test_existing_weekly_job_runs_industry_steps_before_dashboard(self) -> None:
        ...
        self.assertLess(
            scripts_run.index("scripts/run_research_radar_weekly.py"),
            scripts_run.index("scripts/run_future_economy_weekly.py"),
        )
        self.assertLess(
            scripts_run.index("scripts/run_future_economy_weekly.py"),
            scripts_run.index("scripts/build_dashboard.py"),
    ...
    def test_skip_research_radar_omits_only_radar_step(self) -> None:
        ...
        self.assertTrue(any("scripts/run_future_economy_weekly.py" in cmd for cmd in calls))
        self.assertTrue(any("scripts/build_dashboard.py" in cmd for cmd in calls))

    def test_skip_future_economy_omits_only_future_state_step(self) -> None:
        calls: list[list[str]] = []
        ...
        with (
            patch.object(
                sys,
                "argv",
                ["sync_weekly.py", "--skip-stocks", "--skip-future-economy"],
            ),
            patch.object(sync_weekly.subprocess, "run", side_effect=fake_run),
            patch("builtins.print"),
        ):
        ...
        self.assertTrue(any("scripts/run_research_radar_weekly.py" in cmd for cmd in calls))
        self.assertFalse(any("scripts/run_future_economy_weekly.py" in cmd for cmd in calls))
        self.assertTrue(any("scripts/build_dashboard.py" in cmd for cmd in calls))
