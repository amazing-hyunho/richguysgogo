...
SCRIPT = ROOT / "scripts" / "send_industry_cycle_alerts.py"
DB_PATH = ROOT / "data" / "investment.db"


class SendIndustryCycleAlertsCliDryRunTests(unittest.TestCase):
    """Exercises the real CLI entrypoint in dry-run mode only (no --execute),
    so this test can never write to the real `data/investment.db` or send
    ...
    def test_default_invocation_is_dry_run_and_has_no_side_effects(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "--as-of", "2026-07-25"],
            cwd=str(ROOT),
            capture_output=True,
            text=True,
            timeout=60,
        ...
        self.assertIn("send_industry_cycle_alerts_dry_run_only", result.stdout)
