...
SCRIPT = ROOT / "scripts" / "backfill_industry_indicators.py"
DB_PATH = ROOT / "data" / "investment.db"


class BackfillIndustryIndicatorsCliDryRunTests(unittest.TestCase):
    """Exercises the real CLI entrypoint in dry-run mode only (no --execute),
    so this test can never write to the real `data/investment.db` or make
    real network calls."""
    ...
    def test_default_invocation_is_dry_run_and_has_no_side_effects(self) -> None:
        ...
        self.assertIn("backfill_industry_indicators_dry_run_only", result.stdout)
