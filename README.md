...
기본은 배치 중심이며, 추가로 텔레그램 인바운드 Q&A 봇을 폴링 방식으로 실행할 수 있습니다.
...
예시:
```
python scripts/run_nightly.py --build-dashboard
```
...
### 3-1) 텔레그램 채팅 Q&A 봇 실행 (폴링)
```
python scripts/run_bot.py
```
- 인바운드 메시지를 받아 최신 `runs/YYYY-MM-DD/`의 `snapshot.json`, `report.md`, `committee_result.json`을 근거로 답변합니다.
...
```
python scripts/run_news_hourly.py --target-total 300 --top-n 5
```
...
- 기본값으로 지표 수집/DB upsert도 함께 수행합니다(`--collect-indicators`, 기본: 켬).
- 기본값으로 대시보드 재생성도 수행합니다(`--build-dashboard`, 기본: 켬).
- 저장 경로:
  - 최신본: `runs/news/latest_news_digest.json`
  - 이력: `runs/news/history.jsonl`

### 4-2) 뉴스 크롤링 시 자동 커밋 + 자동 푸시
```
python scripts/run_news_hourly.py --target-total 300 --top-n 5
```
- 기본 동작은 자동 커밋 + 자동 푸시입니다.
- 변경 파일(`runs/news/latest_news_digest.json`, `runs/news/history.jsonl`, `data/investment.db`, `docs/dashboard.html`)만 자동 `git add` 후 커밋/푸시합니다.
- 변경이 없으면 커밋/푸시를 건너뜁니다.
- 끄기 옵션: `--no-auto-commit`, `--no-auto-push`, `--no-collect-indicators`
...
$python = "python"
$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoProfile -Command `"cd '$project'; $python scripts/run_news_hourly.py --target-total 300 --top-n 5`""
...
### 로컬 검증 순서 (Windows / macOS / Linux 공통)
아래 순서대로 실행하면 배치 산출물 생성 → 아침 발송 → 인바운드 Q&A를 로컬에서 연속 확인할 수 있습니다.
