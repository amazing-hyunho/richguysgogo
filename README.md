...
- 변경 파일(`runs/news/latest_news_digest.json`, `runs/news/history.jsonl`, `docs/dashboard.html`)만 자동 `git add` 후 커밋/푸시합니다.
- 변경이 없으면 커밋/푸시를 건너뜁니다.
- 끄기 옵션: `--no-auto-commit`, `--no-auto-push`
...
$python = "python"
$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoProfile -Command `"cd '$project'; $python scripts/run_news_hourly.py --target-total 300 --top-n 5 --build-dashboard`""
